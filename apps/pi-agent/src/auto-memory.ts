...
export const reviewTelegramMemory = async (input: AutoMemoryUpdateInput): Promise<AutoMemoryUpdate | undefined> => {
  ...
  const prompt = [
    ...
    '',
    'Memory policy:',
    '- Save only personal info/details and personal preferences, such as name, contact preferences, and writing style.',
    '- Save things the user explicitly asks to remember only if they match that personal-info or personal-preference scope.',
    '- Do not save project/session work, coding details, product decisions, business decisions, plans, or low-value facts.',
