...
import { createApprovalKeyboard } from '../messages/approval';
import { createAuthMiddleware, isAllowedUser } from '../connectors/telegram/auth';
...
import { getAvailablePackages, formatPackagesList } from '../features/packages/packages';
import { resolveSystemPath } from '../core/path';
import { formatServices } from '../server/server';
import { ensureAppDirs, getAppDir, getChatModePath, getPiSdkDir } from '../core/storage';
import { createTaskState, isBusy, popQueuedTask, queueTask } from '../agent/task-state';
import { getChatId, getUserId } from '../connectors/telegram/telegram-context';
...
describe('config and paths', () => {
  ...
  it('parses config and default voice settings', () => {
    ...
    expect(config.server.logFiles).toEqual([resolve(cwd, 'log.txt')]);
  ...
  it('parses custom voice settings', () => {
    ...
    expect(config.voice.whisperModel).toBe(resolve(process.cwd(), 'model.bin'));
  });
  ...
  it('creates app directories', async () => {
    ...
    await writeFile(join(getPiSdkDir(), 'skills', 'ok.txt'), 'ok');
    await expect(readFile(join(getPiSdkDir(), 'skills', 'ok.txt'), 'utf8')).resolves.toBe('ok');
  });
});
...
describe('memory and packages', () => {
  ...
  it('reads package settings safely', async () => {
    ...
    await ensureAppDirs();
    await writeFile(join(getPiSdkDir(), 'settings.json'), JSON.stringify({ packages: ['b', 'a'] }));
    expect(await getAvailablePackages()).toEqual(['a', 'b']);
    expect(await formatPackagesList()).toBe('Packages:\n  - a\n  - b');
    await writeFile(join(getPiSdkDir(), 'settings.json'), JSON.stringify({ packages: [1] }));
    expect(await getAvailablePackages()).toEqual([]);
  });
});
